...
export default function DashboardPage() {
  ...
  // ---- «Сейчас / Далее»: ближайшие события с временем на сегодня ----
  // умная сортировка уже существующих данных, не таймлайн: берём события
  // календаря на сегодня со временем, ещё не наступившие, и показываем два
      ...
      case 'nownext': {
        ...
        return (
          <CollapsibleCard
            ...
            summary={
              up ? (
                <span className="tnum text-xs text-[var(--text-3)]">{relLabel(up.time as string)}</span>
              ) : undefined
            }
          >
            {!up ? (
