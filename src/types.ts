...
export interface BankCard extends SyncStamp {
  ...
  loyalty?: boolean
  /** УСТАРЕЛО, остаётся для совместимости: показывать код штрихкодом.
   *  Читать через lib/cardCode.cardCodeView, писать через cardCodeFields —
   *  поле продолжает заполняться, чтобы карта, открытая на устройстве со
   *  старой сборкой, не оказалась пустой. */
  barcode?: boolean
  /** как показывать код скидочной карты: штрихкод, QR или только цифры.
   *  Отсутствует у карт, заведённых до появления QR — тогда режим выводится
   *  из barcode (см. lib/cardCode). */
  codeView?: 'barcode' | 'qr' | 'none'
